...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for sggCd in seoul:
            cnt = 0
            before_days_num = 60
            while before_days_num > 0:
                before_days = today - timedelta(days=before_days_num)
                before_days_str = before_days.strftime("%Y%m%d")
                data_payload = {"sggCd": sggCd["code"], "beginDate": before_days_str, "endDate": today_str}

                try:
                    result = await fetch_post(session, url, data=data_payload)
                except Exception as e:
                    print(f"{sggCd['code']} {sggCd['kor_name']} {before_days_num} fetch 에러: {str(e)}")
                    result = None
                try:
                    content = json.loads(result) if result else None
                    if content and "result" in content:
                        if len(content["result"]) > 0: break
                except Exception as e:
                    print(f"{sggCd['code']} {sggCd['kor_name']} {before_days_num} json 에러: {str(e)}")
                content = None
                print(f"{sggCd['code']} {sggCd['kor_name']} {before_days_num} 로드 실패")
                before_days_num = before_days_num - 5
            ...
            if content and "result" in content:
                for x in content["result"]:
                    ...
                    if place_name and lat and lng:
                        ...
                        cnt = cnt + 1
            print(f"[{sggCd['code']}] {sggCd['kor_name']} 최근 {before_days_num}일 토지거래허가 {cnt}건")
